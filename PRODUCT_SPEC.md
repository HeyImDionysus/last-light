...
**Gameplay geometry and passability.** Obstacles are gameplay geometry, not decoration. Generate exactly 24 axis-aligned, solid tree-thicket rectangles from the gameplay PRNG before entities. For IDs 0–23, consume candidates in order; each candidate has integer top-left `(80 + rngInt(0, 2,079), 80 + rngInt(0, 1,279))`, width `96 + 16*rngInt(0, 13)`, and height `96 + 16*rngInt(0, 10)`. Accept the first of at most 256 candidates wholly inside the 80 px map margin, whose rectangle expanded by 150 px does not intersect the lighthouse safe circle's bounding box, and whose rectangle expanded by 18 px does not intersect an accepted obstacle expanded by 18 px. The latter is a 36 px **total** gap between the unexpanded rectangles; never expand both rectangles by 36 px. Rejected candidates are still consumed. If any ID has no accepted candidate after 256 attempts, world construction fails its invariant: do not start a partial round, omit that obstacle, renumber later IDs, or substitute a fallback; report the specified game-logic error boundary. Thus a started round always has exactly 24 obstacles with IDs `0..23`.
...
4. The specified safety distances, obstacle/passability rules, spawn deferral behavior, and same-tick terminal ordering are unit tested; no accepted star/shadow may violate them, and generation starts successfully for every deterministic seed from 0 through 999.
