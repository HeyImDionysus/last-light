...
**Respawning stars.** Define `liveStars = availableStars.length + carriedStars.length`; banked stars are not live. On every active tick divisible by 720, if `liveStars < 28`, create one pending respawn request unless one is already pending. A pending request is attempted on its due active tick; initially this is that cadence tick, and after failure it is 60 active ticks later. Normal cadence ticks do not create a second request while one is pending. Each request makes at most 32 attempts using the same two-call coordinate proposal order as initial stars. A proposal is accepted only if it is star-passable, farther than 300 px from the lighthouse, at least 260 px from the current player center, at least 180 px from every active shadow center, more than 100 px from every available star, and outside the deterministic simulation camera rectangle. That rectangle is 960 × 640 world px centered on the player then clamped to the world bounds; CSS viewport, device-pixel ratio, and renderer camera must not affect it. On success, append one available star with `id = nextStarId`, then increment `nextStarId`; `nextStarId` starts at 28 and increments only on a successful respawn. On all 32 failures, consume the candidates, retain the pending request, and defer it exactly 60 active ticks; do not change `liveStars`, IDs, or any safety constraint.

**Shadow spawning.** At every global cadence opportunity `720 + 1,800n` (`n >= 0`), first test whether the round is active and `activeShadows.length < targetShadowCount`; otherwise make no request and consume no gameplay PRNG value. If the test passes, attempt exactly one shadow spawn request on that tick. The request makes at most 32 attempts numbered 1–32. Each attempt consumes exactly two gameplay `rngInt` calls in this order, even if the first coordinate or any later predicate rejects the candidate: `x = 19 + rngInt(0, 2,362)`, then `y = 19 + rngInt(0, 1,562)`. The proposed center is `(x, y)` world pixels. No angle, radius, renderer value, or additional random draw is used.
...
On success, append one active shadow with `id = nextShadowId` and `graceTicks = 21`, then increment `nextShadowId`; `nextShadowId` is initialized to `0` at new-round construction and increments only after a successful shadow spawn, so it is monotonic and never reused. On all 32 failed attempts, consume all 64 `rngInt` calls (and every underlying rejected `nextU32()` draw), create no shadow, leave `nextShadowId` unchanged, and defer that single request to exactly active tick `currentTick + 60`. A deferred request is attempted before a same-tick global cadence test; if it fails, set its next due tick to that attempt tick plus 60. While a request is pending, a global cadence opportunity neither attempts another request nor consumes gameplay PRNG values. Target increases after a cadence opportunity wait until the next opportunity.
...
For each shadow, let `toPlayer = playerCenter - shadowCenter`; if its length is zero, use the exact `shadowId mod 8` fallback vector table in TECHNICAL_DECISIONS.md, otherwise normalize it by that document’s Q16.16 integer algorithm. First call `resolveShadowDisplacement` once with `toPlayerUnit × speed × (1/60)` (pursuit). Then, and only then, evaluate the sole lantern-repulsion predicate for this tick: `lanternEnergy > 0 && distance(playerCenter, shadowCenter) <= 155 px`, using the shadow center after pursuit and lighthouse exclusion. If true, call `resolveShadowDisplacement` exactly once with `-toPlayerUnit × (300 / 60) px`; this is the one 5 px lantern-repulsion displacement for that shadow and tick, including at overlap and everywhere in the full repel range. Set `graceTicks` to 21 whenever this predicate is true, even if terrain shortens the swept displacement. If false, make no lantern-repulsion displacement and do not reset grace. Do not apply lantern repulsion at any other stage. A spawned shadow begins with `graceTicks = 21`; otherwise decrement a positive `graceTicks` once at the end of each subsequent active tick. Grace makes that shadow ineligible to lose the round while positive; it does not stop movement. Shadows never collide with, block, or repel each other.
...
3. Move shadows using the deterministic Shadow simulation, including its one post-pursuit lantern-repulsion predicate/application and safe-zone exclusion.
...
7. Otherwise resolve player–shadow overlaps in ascending shadow ID. This step does not move or repel shadows. A colliding shadow loses the round only when lantern energy is zero and that shadow has zero grace ticks; set `lost` immediately and do not evaluate subsequent IDs. With positive lantern energy, collision causes no additional displacement because the sole lantern-repulsion application was already performed in step 3.
